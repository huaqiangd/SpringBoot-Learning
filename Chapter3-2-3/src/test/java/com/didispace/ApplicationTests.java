...
public class ApplicationTests {
    ...
    public void test() throws Exception {
        ...
        // 往第一个数据源中插入两条数据
        jdbcTemplate1.update("insert into test_USER(id,user_name,age) values(?, ?, ?)", 1, "aaa", 20);
        jdbcTemplate1.update("insert into test_USER(id,user_name,age) values(?, ?, ?)", 2, "bbb", 30);

        // 往第二个数据源中插入一条数据，若插入的是第一个数据源，则会主键冲突报错
        jdbcTemplate2.update("insert into test_USER(id,user_name,age) values(?, ?, ?)", 1, "aaa", 20);
